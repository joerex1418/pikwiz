...
CIVITAI_SAMPLERS = {
    ...
    "euler_ancestral": ["Euler a", "Automatic"],
}
...
def parse_prompt_string(raw_prompt_string):
    ...
    try:
        _json_data = json.loads(raw_prompt_string)
    except json.JSONDecodeError as e:
        # raise e
        console.log("Not raw JSON string")
    
    if _json_data:
        # ==================================== #
        # CivitAI on-site img2img gen
        # ==================================== #
        extra_metadata = json.loads(_json_data["extraMetadata"])
        console.print_json(data=extra_metadata)
        prompts = "{} \nNegative prompt: {}\n".format(extra_metadata["prompt"], extra_metadata["negativePrompt"])
        settings = "Steps: {}, CFG Scale: {}, Sampler: {}, Schedule type: {}, workflowId: {}, civitai resources: {}".format(
            extra_metadata["steps"],
            extra_metadata["cfgScale"],
            CIVITAI_SAMPLERS.get(extra_metadata["sampler"], ['', ''])[0],
            CIVITAI_SAMPLERS.get(extra_metadata["sampler"], ['', ''])[1],
            extra_metadata["workflowId"],
            json.dumps(extra_metadata["resources"])
        )
    else:
    ...
    #     negative_prompt = match.groupdict()["neg"]
    # else:
    #     assert("extraMetadata" in _json_data)

    #     _extrametadata = json.loads(_json_data["extraMetadata"])
    #     console.print_json(data=_extrametadata)
    #     positive_prompt = _extrametadata["prompt"]
    #     negative_prompt = _json_data["negativePrompt"]
    ...
    if settings:
        # pairs = re.findall(r"(\w[\w\s]*?):\s*(\"[^\"]+\"|[^,\n\{]+)", settings, re.DOTALL | re.IGNORECASE)
        ...
        if len(model_version_ids) > 0:
            ...
            for resource in civitai_resources:
                ...
                if len(resource_data) > 0:
                    ...
                    # resource["base_model"] = resource_data[0]["baseModelType"]
